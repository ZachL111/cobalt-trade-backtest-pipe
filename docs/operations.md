...
`cobalt-trade-backtest-pipe` is centered on trading systems work in Zig. The technical target is: Design a Zig verification harness for backtest systems, covering storage recovery, log and snapshot fixtures, and failure-oriented tests.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 5.
